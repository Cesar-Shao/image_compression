...
class KJPEG:
    ...
    def __Encode(self, matrix, tag):
        ...
        for i in range(height // 8):
            for j in range(width // 8):
                res.append(self.__Quantize(self.lost(self.__Dct(blocks[i, j])).reshape(64), tag))
                #res.append(self.lost(self.__Dct(blocks[i, j])).reshape(64))
        ...
        print('Q start')
        f=open('/Users/cesar/Desktop/同步储存/net/jpeg/code_v1/Y_dct_res.txt','w')
        bnum=0
        _num=0
        for i in res:
            bnum+=1
            if(i[1]<0.1 and i[8]<0.1 and i[16]<0.1 and i[9]<0.1 and i[2]<0.1):
                _num+=1
            num=0
            for j in i:
                f.write(str(j))
    ...
    #sjc_uv编码
    def __EncodeUV(self, matrix, tag):
        # 先对矩阵进行填充
        matrix = self.__Fill(matrix)
        # 将图像矩阵切割成8*8小块
        height, width = matrix.shape
        ...
        for i in range(height // 8):
            for j in range(width // 8):
                res.append(self.__Quantize(self.lostUV(self.__Dct(blocks[i, j])).reshape(64), tag))
        #sjc_end
        print('Q start')
        f=open('/Users/cesar/Desktop/同步储存/net/jpeg/code_v1/UV_dct_res.txt','w')
        bnum=0
        _num=0
        for i in res:
            bnum+=1
            if(i[1]<0.1 and i[8]<0.1 and i[16]<0.1 and i[9]<0.1 and i[2]<0.1 and i[8]<0.1):
                _num+=1
            num=0
            for j in i:
                f.write(str(j))
    ...
    def lost(self,block):
        for i in range(8):
            for j in range(8):
                ...
                if((i-j*2+14)%8==a):
                    block[i,j]=0'''
                if not(i*8+j==0 or i*8+j==1 or i*8+j==8 or i*8+j==16 or i*8+j==9 or i*8+j==2 or i*8+j==3 or i*8+j==10):
                #if (i+j>4):
                    block[i,j]=0
    ...
    def Compress(self, filename):
        ...
        u_blocks = self.__EncodeUV(u, self.__ct)
        v_blocks = self.__EncodeUV(v, self.__ct)
        # 对图像小块进行Zig编码和RLE编码
